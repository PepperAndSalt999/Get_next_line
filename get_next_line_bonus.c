...
/*                                                   +#+                      */
/*   Created: 2022/02/25 15:47:14 by rpicot        #+#    #+#                 */
/*   Updated: 2022/02/25 15:47:14 by rpicot        ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
static char	*get_newline(char **store, char *buf)
{
	...
	newline_found = ft_strdup(*store, 0, nl_index - *store + 1);
	free(*store);
	free(buf);
...
static char	*loop_until_empty(int fd, char *buf, char **store, int bytes_read)
{
	...
	while (1)
	{
		ft_strlen_bzero(buf, 2, BUFFER_SIZE + 1);
		if (newline(*store))
